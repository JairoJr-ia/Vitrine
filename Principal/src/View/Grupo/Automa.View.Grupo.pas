...
uses
  ...
  Entidade.GRUPO, Vcl.Imaging.pngimage, System.ImageList,
  Vcl.ImgList,
  math, Automa.View.TransparentDialog, ResizeKit;

type
  TfrmGrupo = class(TfrmBaseCRUD)
    fieldTipo: TRadioGroup;
  ...
  private
    ...
    FGrupo : iControllerDAOGeneric<TGRUPO>;
    CrudGrupo : TGrupo;
...
begin
  inherited;
  FGrupo := TControllerGrupo<TGRUPO>.New;
  FGrupo.DataSource(DSFind);

  CrudGrupo := TGrupo.Create;
...
procedure TfrmGrupo.pnlDeleteClick(Sender: TObject);
var
...
begin
  try
    ...
    begin
      CrudGrupo.CODIGO := DSFind.DataSet.FieldByName('CODIGO').AsInteger;
      FGrupo.Delete(CrudGrupo);
      FGrupo.Find;
    end;
...
procedure TfrmGrupo.pnlFirstPageClick(Sender: TObject);
begin
  inherited;
  if filtro <> '' then
  begin
...
procedure TfrmGrupo.pnlInserirClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmGrupo.pnlLastPageClick(Sender: TObject);
begin
  inherited;
  if filtro <> '' then
  begin
...
procedure TfrmGrupo.pnlNextClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmGrupo.pnlPenutimatePageClick(Sender: TObject);
begin
  inherited;
  if filtro <> '' then
  begin
...
procedure TfrmGrupo.pnlPriorClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmGrupo.pnlSaveClick(Sender: TObject);
begin
  preencherEntidade;
...
procedure TfrmGrupo.pnlSecondPageClick(Sender: TObject);
begin
  inherited;
  if filtro <> '' then
  begin
...
procedure TfrmGrupo.pnlUpdateClick(Sender: TObject);
begin
  inherited;
  fieldNome.SetFocus;
end;

procedure TfrmGrupo.preencherEntidade;
begin
  CrudGrupo.CODIGO := StrToInt(fieldCodigo.Text);
  CrudGrupo.NOME   := fieldNome.Text;
  CrudGrupo.TIPO   := Copy(fieldTipo.Items[fieldTipo.ItemIndex],1,1);

end;

procedure TfrmGrupo.updatePanelPage;
begin
...
procedure TfrmGrupo.updateNumberPage;
var
  FGrupoCount : iControllerDAOGeneric<TGRUPO>;
begin
  FGrupoCount := TControllerGrupo<TGRUPO>.New;
